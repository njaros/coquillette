...
/*   Updated: 2022/03/24 15:26:11 by njaros           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
char	*replace_dollz(char *str, int *i, int end)
{
	...
	sub1 = ft_substr(str, *i + 1, end - (*i + 1));
	replaced = getenv(sub1);
	free(sub1);
	sub1 = ft_substr(str, end, ft_strlen(str));
	sub2 = ft_substr(str, 0, *i - 1);
	*i += ft_strlen(replaced);
	no_dollz = ft_strjoin(sub2, replaced);
	free(sub2);
	free(replaced);
